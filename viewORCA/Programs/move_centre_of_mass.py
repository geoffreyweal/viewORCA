...
class CLICommand:
	...
	def add_arguments(parser):
		...
		parser.add_argument('-c', '--move_com_to', nargs=1, help='This is the (x,y,z) position that you want to move the centre of mass of the system to. If "centre" is given, centre the chemical system. Default=centre', default=['centre'])
	...
	def run(args):
		...
		move_com_to = args.move_com_to
		if (len(move_com_to) == 1) and (move_com_to[0] in ['centre', 'center']):
			move_com_to = 'centre'
		elif (len(move_com_to) != 1):
			...
			toString += f'Input you gave for move_com_to = {move_com_to}'
			exit(toString)
		else:
			move_com_to = move_com_to[0]

		# Third, run the method.
		Run_Method(filepath, move_com_to)

def Run_Method(filepath, move_com_to='centre'):
	"""
	This module allows the user to move the centre of mass of a system. 
	...
	# Third, check move_com_to and obtain the value to move the centre of mass to. 
	if move_com_to == 'centre':
		move_com_to = np.array([0,0,0])
	else:
		try:
			move_com_to = np.array([float(pos) for pos in move_com_to.split(',')])
		except Exception as exception:
			toString  = f'Error, you must give three integers/float values for the centre of mass. These represent the (x,y,z) position that you want to move the centre of mass of your chemical system too.'
			toString += f'Input you gave for move_com_to = {move_com_to}'
			exit(toString)
	if (len(move_com_to) != 3):
		toString  = f'Error, you must give three integers/float values for the centre of mass. These represent the (x,y,z) position that you want to move the centre of mass of your chemical system too.'
		toString += f'Input you gave for move_com_to = {move_com_to}'
		exit(toString)
	...
	# Sixth, get the displacement vector to move the centre of mass - from current_centre_of_mass --to--> move_com_to
	com_displacement = move_com_to - current_centre_of_mass
	...
	new_filepath += filename.replace(suffix,'') + '_com_' + str(move_com_to[0]) + '_' + str(move_com_to[1]) + '_' + str(move_com_to[2]) + suffix
